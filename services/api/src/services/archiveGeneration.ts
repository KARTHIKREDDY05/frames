...
export async function generateDailyFrameForDate(prisma: PrismaClient, userId: string, dateInput: Date) {
  const date = startOfUtcDay(dateInput);
  const next = new Date(date);
  next.setUTCDate(next.getUTCDate() + 1);

  const posts = await prisma.post.findMany({
    where: { userId, deletedAt: null, createdAt: { gte: date, lt: next } },
