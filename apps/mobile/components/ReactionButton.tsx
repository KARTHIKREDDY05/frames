...
export function ReactionButton({ reactions, comments, liked, onReact, onComment, onShare }: { reactions: number; comments: number; liked?: boolean; onReact?: () => void; onComment?: () => void; onShare?: () => void }) {
  return (
    <View style={styles.row}>
      <Pressable style={styles.action} onPress={onReact}>
        <AppIcon name="heart" color={liked ? "#B8324A" : palette.ink} size={20} />
        <Text style={[styles.label, liked && styles.liked]}>{reactions}</Text>
      </Pressable>
      <Pressable style={styles.action} onPress={onComment}>
        <AppIcon name="comment" color={palette.ink} size={20} />
